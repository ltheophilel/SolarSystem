...
int main(int argc, char *argv[])
{
    ...
    /* Parameters */
    Astre Astres[NB_ASTRES] = {0};
    Uint8 colourArray[NB_ASTRES][3] = {
        {255, 255, 255}, {200, 200, 200}, {220, 230, 160},
        {127, 127, 255}, {255, 127, 127}, {255, 255, 220},
        {255, 255, 127}, {127, 255, 210}, {50, 50, 255}};
    SDL_Texture *trajTexture = NULL;
    ...
    double initial_angles[NB_ASTRES] = {0};

    /* Creation */
    init_trajectories(renderer, &trajTexture);
    construct_astres(
        renderer, Astres, radiusArray, distArray, colourArray, initial_angles);

    /* Version Detection */
    void (*update_pos)(
        Astre *Astres, const int *distArray, double reduction_factor);
    switch (dynamics)
    {
    case 0:
    ...
    while (hold == 1)
    {
        ...
        {
            test_text = place_text(
                renderer, &textSurface, &textTexture, font, year_print);
            if (test_text != 0)
                return test_text;
        }
        SDL_Rect textRect = {10, 10, textSurface->w,
        ...
        // FPS Text
        test_text =
            place_text(renderer, &fpsSurface, &fpsTexture, font, fps_array);
        if (test_text != 0)
            return test_text;

        SDL_Rect fpsRect = {WINDOW_WIDTH - fpsSurface->w - 10, 10,
                            fpsSurface->w, fpsSurface->h}; // Position and size
        ...
        {
            fps = 1000 * (nb_frames - nb_frames_checkpoint) /
                  (time_taken - time_checkpoint);
    ...
    quit_universe(
        window, renderer, Astres, trajTexture, textTexture, textSurface,
        fpsTexture, fpsSurface, font);
    double average_time_taken = time_taken / nb_frames;
    printf("Average computing time = %lf ms\n", average_time_taken);
    return 0;
}
