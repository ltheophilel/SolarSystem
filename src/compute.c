 ...
 */
static double norm2(double x, double y) { return sqrt(x * x + y * y); }
 ...
 */
static double
compute_distance_inv_cubed(Astre A1, Astre A2, double reduction_factor)
{
 ...
 * @param Astres
 */
void initial_speed_dynamics(
    Astre *Astres,
    const int distArray[NB_ASTRES],
 ...
 * @param distArray
 */
void update_positions_kinematics(
    Astre *Astres,
    const int *distArray,
 ...
 * @param Astres
 */
void update_positions_dynamics(
    Astre *Astres,
    ...
    double reduction_factor)
{
    ...
    for (int i = 1; i < NB_ASTRES; i++)
    {
        ...
        for (int j = 0; j < NB_ASTRES; j++)
        {
            ...
            {
                double common_part =
                    massArray[j] * compute_distance_inv_cubed(
                                       Astres[i], Astres[j], reduction_factor);

                acceleration_sum_x += common_part *
                                      (Astres[j].x - Astres[i].x) *
                                      reduction_factor;
                acceleration_sum_y += common_part *
                                      (Astres[j].y - Astres[i].y) *
                                      reduction_factor;
            }
        }
        ...
        a->y += a->vy * dt;
        a->angle = atan2(
            -(a->y - (double)WINDOW_HEIGHT / 2),
            a->x - (double)WINDOW_WIDTH / 2);
    }
}
