...
void get_date(int *year, int *month, int *day)
{
    ...
    if (today[4] == ' ')
        ...
        *day = 10 * (today[4] - '0') + (today[5] - '0');

    *year = 1000 * (today[7] - '0') + 100 * (today[8] - '0') +
            10 * (today[9] - '0') + (today[10] - '0');
}
 ...
 * @return int
 */
static int days_past(int year, int month, int day)
{
...
void get_today_angles(double today_angles[NB_ASTRES])
{
    ...
    for (int i = 0; i < NB_ASTRES; i++)
        today_angles[i] =
            angleArray[i] + (nb_days / yearDurationArray[i]) * 2 * PI;
}
